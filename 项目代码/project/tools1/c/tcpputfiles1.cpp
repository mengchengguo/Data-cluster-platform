...
bool ActiveTest(){
  ...
  if(TcpClient.Read(strrecvbuffer, 20) == false) return false;    //接受服务端的回应报文 设置20秒的超时时间
  logfile.Write("接收：%s\n", strrecvbuffer);
...
bool Login(const char* argv){
  memset(strsendbuffer, 0, sizeof(strsendbuffer));   // 接收报文的buffer
  memset(strrecvbuffer, 0, sizeof(strrecvbuffer));   // 发送报文的buffer

  SPRINTF(strsendbuffer, sizeof(strsendbuffer), "%s<clienttype>1</clienttype>", argv);   //拼凑登陆的报文
  logfile.Write("发送：%s\n", strsendbuffer);
  if(TcpClient.Write(strsendbuffer) == false) return false;    //向服务端发送请求报文
...
void _help(){
  ...
  printf("Sample:/project/tools1/bin/procctl 20 /root/project/tools1/bin/tcpputfiles1 /tmp/tcpputfiles_surfdata.log \"<ip>101.43.160.209</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n\n\n");
